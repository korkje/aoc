...
for (let i = 0; i < bricks.length; ++i) {
    ...
    while (quque.length) {
        ...
        for (const k of supporter.get(j)!) {
            if (falling.has(k)) {
                continue;
            }

            const supportedBy = [...supported.get(k)!];

            if (supportedBy.every(_ => falling.has(_))) {
                quque.push(k);
                falling.add(k);
            }
        }
