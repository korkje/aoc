...
const paths = (keypad: Keypad, a: string, b: string) => {
...
const _cost = M((keypad: Keypad, a: string, b: string, n: number) => {
    let min: number = Infinity;

    for (const path of paths(keypad, a, b)) {
...
for (const code of file.split("\n")) {
    answer += cost(numeric, code, 25 + 1) * Number(code.slice(0, -1));
}
